...
module.exports = class User {
    ...
    static async findById(id) {
        ...
        if (rows[0].likes[0] !== null) {
            rows[0].likes = rows[0].likes.map(like => new Like(like))
        } else {
            rows[0].likes = []
        }
    ...
    static async findSubscriptionsByEmail(userEmail) {
        ...
        SELECT json_agg(json_build_object(
            ...
            'likeCount', (SELECT COUNT(likes.id) FROM likes WHERE likes.post_id = posts.id),
            'commentCount', (SELECT COUNT(comments.id) FROM comments WHERE comments.post_id = posts.id)
...
<<<<<<< HEAD
        ...
        // if (rows[0].posts[0].id === null) {
        //     rows[0].posts = []
        // }

=======
>>>>>>> e0025ded84a4a059823c643a8ebe93c82b793f7f
