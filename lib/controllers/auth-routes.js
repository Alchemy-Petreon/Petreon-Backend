    ...
    .get('/google/callback',
        ...
        function (req, res) {
            try {
                ...
                res.redirect(
                    `${process.env.CLIENT_SIDE_REDIRECT}
                    ${req._passport.session.user.email}
                    /${req._passport.session.user.firstName}
                    /${req._passport.session.user.exisiting}`)
