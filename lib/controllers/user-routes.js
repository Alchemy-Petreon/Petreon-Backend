...
module.exports = Router()
    ...
    .get(`/subscriptions/:id`, ensureAuth, async (req, res, next) => {
        User
            .findOneSubscription(req.user, req.params.id)
            .then(subscribed => res.send(subscribed))
            .catch(next)
    })
    ...
    .get('/unsubscribe/:id', ensureAuth, async (req, res, next) => {
        const userInfo = await User.findByEmail(req.user)
        await User.unsubscribe(userInfo.id, req.params.id)
        res.send(200)
